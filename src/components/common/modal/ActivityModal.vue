  ...
  >
    ...
    <div class="px-2 pb-2 sm:px-8 sm:pb-8 w-full">
      <div class="w-full text-center">
        <div class="font-oswald text-sm sm:text-xl font-medium uppercase my-2">
          {{ data.name }}
        </div>
        ...
        <div class="flex justify-center w-full mt-4">
          <div
            class="text-sm sm:text-xl px-4 py-2 w-1/2 sm:w-1/3 md:w-1/4 font-bold cursor-pointer"
            :class="[pre.bg + color.c1, pre.text + color.c3]"
            @click="gotoLink()"
          >
            <a @click.self.prevent class="w-full h-full" :href="data.url"
              >Go to link</a
            >
            <a
              :id="'url_activity' + index"
              target="_blank"
              :href="data.url"
              hidden
            ></a>
